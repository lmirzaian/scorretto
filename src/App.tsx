...
import { clearSavedRoom, loadSavedRoom, saveRoom } from './utils/storage';
...
export default function App() {
  const [state, setState] = useState<GameState>(() => loadSavedRoom() ?? initialState);
  const [timerKey, setTimerKey] = useState(0);
  useEffect(() => saveRoom(state), [state]);
  ...
  if (state.gamePhase === 'victory') return <VictoryScreen {...state} winnerName={winnerName} rounds={state.currentRoundNumber} onRestart={() => { clearSavedRoom(); setState(initialState); }} onOpenReport={() => setState((p) => ({ ...p, gamePhase: 'testReport' }))} />;
  if (state.gamePhase === 'home') return <section className="container"><article className="card hero"><p className="label">Stanza offline locale</p><h1>Non si può più dire niente</h1><button onClick={() => setState((p) => ({ ...p, gamePhase: 'setup' }))}>Nuova stanza offline</button>{state.roundHistory.length ? <button className="secondary" onClick={() => setState((p) => ({ ...p, gamePhase: p.teamAScore >= p.targetScore || p.teamBScore >= p.targetScore ? 'victory' : 'playing' }))}>Riprendi stanza salvata</button> : null}<button className="secondary" onClick={() => { clearSavedRoom(); setState(initialState); }}>Cancella stanza salvata</button>{state.roundHistory.length ? <button className="secondary" onClick={() => setState((p) => ({ ...p, gamePhase: 'testReport' }))}>Report test serata</button> : null}</article></section>;
